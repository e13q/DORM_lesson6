...
def serialize_post_without_comments(post):
    return {
        'title': post.title,
        'teaser_text': post.text[:200],
        'author': post.author.username,
...
def index(request):
    ...
    most_popular_posts = annotate_comments_count(most_popular_posts)
    most_fresh_posts = Post.objects.order_by(
        '-published_at'
    )[:5].prefetch_related('author')
    popular_tags = Tag.objects.annotate(
        num_tags=Count('posts')
    ).order_by('-num_tags')
    ...
    context = {
        ...
        'page_posts': [serialize_post_without_comments(post) for post in most_fresh_posts],
